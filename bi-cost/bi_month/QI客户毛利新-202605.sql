-- QI客户毛利2026-05（完整版，增加KYC应收金额已去除，增加DCSF成本，手动实收）
-- VISA另一份账单、活跃卡费、还没出
SET myapp.start_time = '2026-05-01 00:00:00';
SET myapp.end_time = '2026-06-01 00:00:00';
SET myapp.param_a = '0.9808';
SET myapp.param_b = '0.9525';
SET myapp.param_c = '1.1040';
SET myapp.param_d = '1.1785'; --VRM
SET myapp.param_e = '0.9945'; --CROSS
SET myapp.param_f = '1.0563'; --DCSF
...
receivable_allocation AS (
    WITH bill_ids AS (
        ...
        WHERE delete_time IS NULL AND is_latest='t' AND bill_month='2026-05'
    ),
    summary AS (
...
new_receipts AS (
    ...
    WHERE A."Collection Month" = '2026-05' 
      AND A."Provider" = 'IQ'
...
combined_costs AS (
    SELECT 
        ...
        CASE 
            WHEN cc."母客户id" = '666245' THEN 11561.61
...
ProviderTopUpFee AS (
    ...
    CROSS JOIN params p
    WHERE ptf."Collection Month" = '2026-05'   -- 按您的要求使用2026-05
...
final_summary AS (
    SELECT 
        ...
        GREATEST((21546 + 10667+0) * COALESCE(cons."客户QI渠道净消费占比", 0) / 100, 0) as "成本十",
        GREATEST(8500 * COALESCE(cons."客户QI渠道净消费占比", 0) / 100, 0) as "银行手续费成本",
        (CASE WHEN amc."母客户id" = '732600' THEN -0 ELSE 0 END)* (COALESCE(cons."QI渠道净消费占比", 0) / 100) AS "直客手动退款",
        (CASE 
            WHEN amc."母客户id" = '621311' THEN -34.62
            WHEN amc."母客户id" = '413969' THEN -4340.93
        END )* (COALESCE(cons."QI渠道净消费占比", 0) / 100) AS "应收_财务线下手动调整",
        CASE 
            WHEN amc."母客户id" = '440126' THEN 1200
            ELSE 0
        END AS "线下实体卡收入",
        CASE 
            WHEN amc."母客户id" = '018142' THEN 7.84
            WHEN amc."母客户id" = '091450' THEN 16.84
            WHEN amc."母客户id" = '238996' THEN 10.84
            WHEN amc."母客户id" = '744345' THEN 7.84
            WHEN amc."母客户id" = '835152' THEN 10.84
            WHEN amc."母客户id" = '907519' THEN 79.84
            WHEN amc."母客户id" = '941375' THEN 28.84
            WHEN amc."母客户id" = '099990' THEN 7.84
            WHEN amc."母客户id" = '289420' THEN 16.84
            WHEN amc."母客户id" = '369660' THEN 19.84
            WHEN amc."母客户id" = '665218' THEN 10.84
            WHEN amc."母客户id" = '701880' THEN 2704.84
            WHEN amc."母客户id" = '711439' THEN 577.84
            WHEN amc."母客户id" = '916785' THEN 4510.84
            WHEN amc."母客户id" = '990552' THEN 40.84
            WHEN amc."母客户id" = '440126' THEN 807.84
            ELSE 0
        END AS "新增实体卡成本",
        (CASE 
            WHEN amc."母客户id" = '637549' THEN 0
            ELSE 0
        END) * (COALESCE(cons."QI渠道净消费占比", 0) / 100) AS "财务手动调整返现成本",
        CASE 
            WHEN amc."母客户id" = '128139' THEN 162.5
            WHEN amc."母客户id" = '322383' THEN 1295.71
            WHEN amc."母客户id" = '436731' THEN 26.71
            WHEN amc."母客户id" = '523135' THEN 27.31
            WHEN amc."母客户id" = '599703' THEN 622.78
            WHEN amc."母客户id" = '637549' THEN 168.75
            WHEN amc."母客户id" = '838108' THEN 15890.62
            WHEN amc."母客户id" = '990552' THEN 2.33
            ELSE 0
        END AS "客户返现代收成本",
        CASE 
            WHEN amc."母客户id" = '183821' THEN 14159.18
            WHEN amc."母客户id" = '990552' THEN 0.7
            ELSE 0
        END AS "手动计算应收",
...
SELECT 
    ...
    -- Pref_Rev_本月月账单_not collected：移除低消和月费应收
    (COALESCE(f."其他手续费应收金额_QI", 0) + 
       COALESCE(f."手动计算应收", 0) + COALESCE(f."不可拆分应收", 0) + COALESCE(f."应收_财务线下手动调整", 0)
    ) AS "Pref_Rev_本月月账单_not collected",
    ...
    -- Cashflow_Rev_之前月账单_collected：移除低消和月费实收
    ...
    -- Cashflow_GP：基于修改后的Cashflow_Rev
    ( (COALESCE(f."充值费收入_QI", 0) + COALESCE(f."虚拟开卡费收入", 0) + 
     COALESCE(f."实体卡制卡费收入", 0) + COALESCE(f."FX_Cross费用收入", 0) + 
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."邮寄费收入", 0) + COALESCE(f."直客手动退款", 0) +
    ...
    -- 销售调整版本（低消月费本身未包含，无需改动，但为了一致性，保留原表达式）
    (COALESCE(f."充值费收入_QI", 0) + COALESCE(f."虚拟开卡费收入", 0) + COALESCE(f."FX_Cross费用收入", 0) + 
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."直客手动退款", 0) + COALESCE(f."其他手续费实收金额_QI", 0)) AS "Cashflow_Rev_销售调整",

    (COALESCE(f."充值费收入_QI", 0) + COALESCE(f."虚拟开卡费收入", 0) + COALESCE(f."FX_Cross费用收入", 0) + 
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."直客手动退款", 0)) AS "Cashflow_Rev_realtime_销售调整",

    COALESCE(f."其他手续费实收金额_QI", 0) AS "Cashflow_Rev_之前月账单_collected_销售调整",
		 
    (COALESCE(f."成本一", 0) + COALESCE(f."成本二", 0) + COALESCE(f."成本三", 0) + 
     COALESCE(f."成本四", 0) + COALESCE(f."成本五", 0) + COALESCE(f."成本六", 0) + 
     COALESCE(f."成本七", 0) + COALESCE(f."成本八", 0) + COALESCE(f."成本九", 0) + 
     COALESCE(f."成本十", 0) + COALESCE(f."银行手续费成本", 0) + 
     COALESCE(f."客户返现代收成本", 0) + COALESCE(f."财务手动调整返现成本", 0)) AS "Cashflow_Cost_销售调整",

    ( (COALESCE(f."充值费收入_QI", 0) + COALESCE(f."虚拟开卡费收入", 0) + COALESCE(f."FX_Cross费用收入", 0) + 
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."直客手动退款", 0) + COALESCE(f."其他手续费实收金额_QI", 0)) -
     (COALESCE(f."成本一", 0) + COALESCE(f."成本二", 0) + COALESCE(f."成本三", 0) + 
     COALESCE(f."成本四", 0) + COALESCE(f."成本五", 0) + COALESCE(f."成本六", 0) + 
     COALESCE(f."成本七", 0) + COALESCE(f."成本八", 0) + COALESCE(f."成本九", 0) + 
     COALESCE(f."成本十", 0) + COALESCE(f."银行手续费成本", 0) + 
     COALESCE(f."客户返现代收成本", 0) + COALESCE(f."财务手动调整返现成本", 0))
     ) AS "Cashflow_GP_销售调整"
...
WHERE f."母客户id" IS NOT NULL
    AND (
        ...
        OR f."新增收入" != 0 OR f."低消实收金额_QI" != 0 OR f."月结手续费实收金额_QI" != 0 
        OR f."其他手续费实收金额_QI" != 0 OR f."总实收金额_QI" != 0 OR f."总实收金额_所有渠道" != 0 OR f."邮寄费收入" != 0 
        OR f."低消应收金额_QI" != 0 OR f."月结手续费应收金额_QI" != 0 
