...
CROSS JOIN LATERAL (
  ...
  WHERE sale_or_am_id IS NOT NULL
    ...
    SELECT CAST("account_id" AS text) AS "account_id", CAST(ids."sale_or_am_id" AS text) AS "sale_or_am_id", CAST("status" AS text) AS "status", CAST("sender_type" AS text) AS "sender_type", CAST("recipient_type" AS text) AS "recipient_type", CAST(COUNT(*) AS integer) AS "transaction_count", CAST(SUM("origin_amount" * "usd_rate") AS numeric(18,2)) AS "origin_amount", CAST(SUM("settlement_amount" * "usd_rate") AS numeric(18,2)) AS "settlement_amount", CAST(SUM("fee" * "usd_rate") AS numeric(18,2)) AS "fee", CAST(SUM("fee2" * "usd_rate") AS numeric(18,2)) AS "fee2", CAST(SUM("cross_chain_fee" * "usd_rate") AS numeric(18,2)) AS "cross_chain_fee", CAST(SUM(CASE WHEN tr."status" = ''Closed'' AND tr."action" = ''sell'' AND tr.hidden = FALSE AND (tr."fee" - tr."origin_amount" * 0.0009) > 0 THEN (tr."fee" - tr."origin_amount" * 0.0009) ELSE 0 END) AS numeric(18,2)) AS "exchange_profit", CAST(SUM(CASE WHEN tr."recipient_type" IN (''wire'',''outside_bank'') AND tr."status" IN (''Processing'',''Closed'') AND (tr."fee" - 25) > 0 THEN (tr."fee" - 25) ELSE 0 END) AS withdraw_fee_diff AS numeric(18,2)) AS "payment_profit", "hidden" AS "hidden", tr."create_time"::DATE::TIMESTAMP AS "create_date", CAST("currency" AS text) AS "currency", CAST("action" AS text) AS "action", CAST(1 AS integer) AS "version", NOW() AS "create_time", NOW() AS "update_time"
