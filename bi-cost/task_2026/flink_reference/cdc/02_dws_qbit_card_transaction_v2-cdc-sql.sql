...
-- Description:    dws_qbit_card_transaction 流处理(CDC) 作业（quantum-v2 范式：确定性哈希主键 + 先清后写）
-- 作业元信息：
--   作业类型：流处理(CDC)
...
CREATE TEMPORARY TABLE source_delete_dws_qbit_card_transaction_result (
    ...
    'table-name' = '(SELECT public.fn_delete_dws_qbit_card_transaction_cdc(false) AS affected_rows) AS delete_result',
...
LEFT JOIN "qbitCard" AS qc ON tr."cardId" = qc."id"
    ...
    SELECT CAST(tr."accountId" AS text) AS "account_id", CAST(tr."businessType" AS text) AS "business_type", CAST(tr."status" AS text) AS "status", CAST(tr."provider" AS text) AS "provider", CAST(qc."firstSix" AS text) AS "bin", CAST(COALESCE(SUM(tr."originalAmount"), 0) AS numeric(18,2)) AS "origin_amount", CAST(COALESCE(SUM(tr."settleAmount"), 0) AS numeric(18,2)) AS "settle_amount", CAST(COUNT(*) AS integer) AS "transaction_count", CAST(COALESCE(SUM(tr."fee"), 0) AS numeric(18,2)) AS "fee", tr."createTime"::DATE::TIMESTAMP AS "create_date", CAST(1 AS integer) AS "version", NOW() AS "create_time", NOW() AS "update_time"
    FROM "qbit_card_transaction" AS tr
LEFT JOIN "qbitCard" AS qc ON tr."cardId" = qc."id"
...
--    （source_dws_qbit_card_transaction 已输出聚合列，此处只补 id；列名与 DWS 表一致）
-- ==============================================
...
) WITH ('connector'='adbpg','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}','tableName'='dws_qbit_card_transaction_2024','userName'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','writeMode'='upsert','batchSize'='2000');
CREATE TEMPORARY TABLE sink_dws_qbit_card_transaction_2025 (
    id BIGINT, account_id STRING, business_type STRING, status STRING, provider STRING, bin STRING, origin_amount DECIMAL(18,2), settle_amount DECIMAL(18,2), transaction_count INT, fee DECIMAL(18,2), create_date TIMESTAMP(6), version INT, create_time TIMESTAMP(6), update_time TIMESTAMP(6),
    PRIMARY KEY (id) NOT ENFORCED
) WITH ('connector'='adbpg','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}','tableName'='dws_qbit_card_transaction_2025','userName'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','writeMode'='upsert','batchSize'='2000');
