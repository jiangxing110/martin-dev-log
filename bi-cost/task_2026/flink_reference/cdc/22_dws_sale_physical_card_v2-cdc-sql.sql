...
CREATE TEMPORARY TABLE source1_transaction (transaction_id STRING,account_id STRING,provider STRING,bin STRING,status STRING,origin_amount DECIMAL(18,2),create_time TIMESTAMP(6),delete_time TIMESTAMP(6)) WITH ('connector'='jdbc','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}?stringtype=unspecified','table-name'='(SELECT CAST(tr.id AS text),CAST(tr."accountId" AS text),CAST(qc."provider" AS text),CAST(qc."firstSix" AS text),CAST(tr."status" AS text),CAST(tr."originAmount" AS numeric(18,2)),tr."createTime",tr."deleteTime" FROM "qbitCardWalletTransaction" tr LEFT JOIN "qbitCard" qc ON tr."cardId"=qc."id" WHERE ((tr."createTime">=CURRENT_DATE-INTERVAL ''1 day'' AND tr."createTime"<CURRENT_DATE) OR (tr."updateTime">=CURRENT_DATE-INTERVAL ''1 day'' AND tr."updateTime"<CURRENT_DATE) OR (tr."deleteTime">=CURRENT_DATE-INTERVAL ''1 day'' AND tr."deleteTime"<CURRENT_DATE)) AND tr."businessType"=''TransferOut'' AND tr."remarks" IN (''邮寄费'',''制卡费'',''批量邮寄运费'')) tx','username'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','driver'='org.postgresql.Driver','scan.fetch-size'='2000');
