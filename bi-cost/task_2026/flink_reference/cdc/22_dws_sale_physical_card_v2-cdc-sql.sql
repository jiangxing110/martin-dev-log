...
-- Description:    dws_sale_physical_card 流处理(CDC) 作业（quantum-v2 范式：确定性哈希主键 + 先清后写）
-- 作业元信息：
--   作业类型：流处理(CDC)
...
-- 多 Source：昨日实体卡交易 source1，销售/AM关系 source2，匹配和聚合在 Flink 算子层完成
CREATE TEMPORARY TABLE source1_transaction (transaction_id STRING,account_id STRING,provider STRING,bin STRING,status STRING,origin_amount DECIMAL(18,2),create_time TIMESTAMP(6),delete_time TIMESTAMP(6)) WITH ('connector'='jdbc','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}?stringtype=unspecified','table-name'='(SELECT CAST(tr.id AS text) AS transaction_id,CAST(tr."accountId" AS text) AS account_id,CAST(qc."provider" AS text) AS provider,CAST(qc."firstSix" AS text) AS bin,CAST(tr."status" AS text) AS status,CAST(tr."originAmount" AS numeric(18,2)) AS origin_amount,tr."createTime" AS create_time,tr."deleteTime" AS delete_time FROM "qbitCardWalletTransaction" tr LEFT JOIN "qbitCard" qc ON tr."cardId"=qc."id" WHERE ((tr."createTime">=CURRENT_DATE-INTERVAL ''1 day'' AND tr."createTime"<CURRENT_DATE) OR (tr."updateTime">=CURRENT_DATE-INTERVAL ''1 day'' AND tr."updateTime"<CURRENT_DATE) OR (tr."deleteTime">=CURRENT_DATE-INTERVAL ''1 day'' AND tr."deleteTime"<CURRENT_DATE)) AND tr."businessType"=''TransferOut'' AND tr."remarks" IN (''邮寄费'',''制卡费'',''批量邮寄运费'')) tx','username'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','driver'='org.postgresql.Driver','scan.fetch-size'='2000');
...
) WITH ('connector'='adbpg','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}','tableName'='dws_sale_physical_card_2024','userName'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','writeMode'='upsert','batchSize'='2000');
CREATE TEMPORARY TABLE sink_dws_sale_physical_card_2025 (
    id BIGINT, account_id STRING, sale_or_am_id STRING, provider STRING, bin STRING, status STRING, transaction_count INT, physical_card_fee DECIMAL(18,2), create_date TIMESTAMP(6), version INT, create_time TIMESTAMP(6), update_time TIMESTAMP(6),
    PRIMARY KEY (id) NOT ENFORCED
) WITH ('connector'='adbpg','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}','tableName'='dws_sale_physical_card_2025','userName'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','writeMode'='upsert','batchSize'='2000');
