...
-- Description:    dws_qbit_card_wallet_transaction 批处理 作业（quantum-v2 范式：确定性哈希主键 + 先清后写）
-- 作业元信息：
--   作业类型：批处理
...
CREATE TEMPORARY TABLE source_delete_dws_qbit_card_wallet_transaction_result (
    ...
    'table-name' = '(SELECT public.fn_delete_dws_qbit_card_wallet_transaction_cdc(false, CAST(''${start_date}'' AS DATE), CAST(''${end_date}'' AS DATE)) AS affected_rows) AS delete_result',
...
CREATE TEMPORARY TABLE source_dws_qbit_card_wallet_transaction (
    ...
    SELECT CAST(tr."accountId" AS text) AS "account_id", CAST(tr."businessType" AS text) AS "business_type", CAST(tr."status" AS text) AS "status", CAST(COALESCE(SUM(tr."originAmount"), 0) AS numeric(18,2)) AS "origin_amount", CAST(COUNT(*) AS integer) AS "transaction_count", CAST(COALESCE(SUM(tr."fee"), 0) AS numeric(18,2)) AS "fee", tr."createTime"::DATE::TIMESTAMP AS "create_date", CAST(1 AS integer) AS "version", NOW() AS "create_time", NOW() AS "update_time"
    FROM "qbitCardWalletTransaction" AS tr
