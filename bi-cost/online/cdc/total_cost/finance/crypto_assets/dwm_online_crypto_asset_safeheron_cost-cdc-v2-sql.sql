...
-- Description:    金融渠道成本 DWM CDC 初始化 - CRYPTO_ASSET / Safeheron v2
-- 作业元信息：
--   作业类型：CDC
...
-- Notes:
--   1. v2 在同一个 Flink SQL 作业中通过 JDBC source 调用 dwm.fn_delete_finance_crypto_asset_safeheron_cost_cdc(false) 先清理目标数据。
...
CREATE TEMPORARY TABLE source_delete_finance_crypto_asset_safeheron_cost_cdc_result (
    affected_rows BIGINT
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dwm.fn_delete_finance_crypto_asset_safeheron_cost_cdc(false) AS affected_rows) AS delete_result',
...
CREATE TEMPORARY TABLE source_crypto_blockchain_transfers (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}?stringtype=unspecified',
    'table-name' = '(SELECT bt.account_id, bt.action, bt.create_time, bt.status, bt.platform FROM ods.view_crypto_assets_blockchain_transfers bt WHERE EXISTS (SELECT 1 FROM (SELECT DISTINCT DATE_TRUNC(''month'', statistics_time)::date AS source_month, (DATE_TRUNC(''month'', statistics_time)::date + INTERVAL ''1 month'')::date AS next_month FROM ods.ods_bi_month_tag WHERE delete_time IS NULL AND product_line = ''CRYPTO_ASSET'' AND provider = ''Safeheron'' AND update_time >= (CURRENT_DATE - INTERVAL ''1 day'')::timestamp AND update_time < CURRENT_DATE::timestamp) p WHERE bt.create_time >= p.source_month::timestamp AND bt.create_time < p.next_month::timestamp)) AS crypto_blockchain_transfers_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000'
...
CROSS JOIN source_delete_finance_crypto_asset_safeheron_cost_cdc_result AS delete_result
