...
-- 历史名称：sp_init_crypto_asset_tz_cost.sql
-- Description:    金融渠道成本 DWM CDC 初始化 - CRYPTO_ASSET / TZ (TZ-wire / TZ-usdt / TZ-usdc) v2
-- 作业元信息：
--   作业类型：CDC
...
-- Notes:
--   1. v2 在同一个 Flink SQL 作业中通过 JDBC source 调用 dwm.fn_delete_finance_crypto_asset_tz_cost_cdc(false) 先清理目标数据。
--   2. 删除范围按昨天更新的 ods_bi_month_tag 推导 source_month，并用 product_line/provider/cost_type/remarks 收窄。
--   3. 部署时需要在“附加依赖文件”添加 PostgreSQL JDBC driver，例如 postgresql-42.7.4.jar。
--   4. 首次执行可将函数参数 false 改为 true 做 dry-run。
--********************************************************************--
-- 作业类型：CDC (默认昨天)
...
CREATE TEMPORARY TABLE source_delete_finance_crypto_asset_tz_cost_cdc_result (
    affected_rows BIGINT
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dwm.fn_delete_finance_crypto_asset_tz_cost_cdc(false) AS affected_rows) AS delete_result',
...
CREATE TEMPORARY TABLE source_crypto_assets_transfers (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}?stringtype=unspecified',
    'table-name' = '(SELECT t.id, t.account_id, t.recipient_type, t.status, t.action, t.currency, t.origin_amount, t.usd_rate, t.extend_field, t.create_time, t.delete_time FROM ods.ods_crypto_assets_transfers t WHERE t.delete_time IS NULL AND EXISTS (SELECT 1 FROM (SELECT DISTINCT DATE_TRUNC(''month'', statistics_time)::date AS source_month, (DATE_TRUNC(''month'', statistics_time)::date + INTERVAL ''1 month'')::date AS next_month FROM ods.ods_bi_month_tag WHERE delete_time IS NULL AND product_line = ''CRYPTO_ASSET'' AND provider IN (''TZ-wire'', ''TZ-usdt'', ''TZ-usdc'') AND update_time >= (CURRENT_DATE - INTERVAL ''1 day'')::timestamp AND update_time < CURRENT_DATE::timestamp) p WHERE t.create_time >= p.source_month::timestamp AND t.create_time < p.next_month::timestamp)) AS crypto_assets_transfers_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000'
...
GROUP BY p.source_month, t.account_id, CAST(t.create_time AS DATE)
...
WHERE t.recipient_type = 'wire'
  ...
  AND t.delete_time IS NULL
  AND JSON_VALUE(t.extend_field, '$.platform') = 'TZ'
GROUP BY p.source_month, t.account_id;
...
GROUP BY p.source_month, t.account_id, t.currency, CAST(t.create_time AS DATE)
HAVING CAST(SUM(COALESCE(t.origin_amount, CAST(0 AS DECIMAL(20, 4))) * COALESCE(t.usd_rate, CAST(0 AS DECIMAL(20, 8)))) AS DECIMAL(20, 4)) <> CAST(0 AS DECIMAL(20, 4));
...
SELECT * FROM v_tz_wire_fee_basis
...
SELECT
    ...
    b.cost_amount,
    1 AS version,
    CAST('crypto_asset_tz_cdc' AS STRING) AS remarks,
...
SELECT v.*
FROM v_dwm_finance_channel_cost AS v
CROSS JOIN source_delete_finance_crypto_asset_tz_cost_cdc_result AS delete_result
