...
--     AND product_line = 'ACQUIRING'
--     AND delete_time IS NULL;
-- Notes:
--   1. v2 在同一个 Flink SQL 作业中通过 JDBC source 调用 dwm.fn_delete_finance_acquiring_cost_cdc(false) 先清理目标数据。
...
CREATE TEMPORARY TABLE source_delete_finance_acquiring_cost_cdc_result (
    affected_rows BIGINT
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dwm.fn_delete_finance_acquiring_cost_cdc(false) AS affected_rows) AS delete_result',
...
-- Orenda: raw_cost = acquiring_usd_amount * 0.0025
...
GROUP BY p.source_month, ac.account_id, ac.create_date
...
SELECT
    ...
    b.cost_amount,
    1 AS version,
    CAST('acquiring_cdc' AS STRING) AS remarks,
    CAST(CURRENT_TIMESTAMP AS TIMESTAMP(6)) AS create_time,
    CAST(CURRENT_TIMESTAMP AS TIMESTAMP(6)) AS update_time,
    CAST(NULL AS TIMESTAMP(6)) AS delete_time
FROM v_allocated_cost_base b
LEFT JOIN source_dim_account da
    ON da.id = b.account_id
...
CROSS JOIN source_delete_finance_acquiring_cost_cdc_result AS delete_result
