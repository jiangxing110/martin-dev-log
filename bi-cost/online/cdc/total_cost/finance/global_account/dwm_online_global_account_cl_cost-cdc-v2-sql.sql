...
-- 历史名称：sp_init_global_account_cl_cost.sql
-- Description:    金融渠道成本 DWM CDC 初始化 - GLOBAL_ACCOUNT / CL (Column) v2
-- 作业元信息：
--   作业类型：CDC
...
-- Notes:
--   1. v2 在同一个 Flink SQL 作业中通过 JDBC source 调用 dwm.fn_delete_finance_global_account_cl_cost_cdc(false) 先清理目标数据。
...
CREATE TEMPORARY TABLE source_delete_finance_global_account_cl_cost_cdc_result (
    affected_rows BIGINT
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dwm.fn_delete_finance_global_account_cl_cost_cdc(false) AS affected_rows) AS delete_result',
...
SELECT
    ...
    b.cost_amount,
    1 AS version,
    CAST('global_account_cdc' AS STRING) AS remarks,
...
CROSS JOIN source_delete_finance_global_account_cl_cost_cdc_result AS delete_result
