...
-- Description:    注册 BB 量子卡 monthly-cdc PostgreSQL 删除任务
-- Schedule:       每月 8 日北京时间 02:00
-- Notes:
--   1. 在目标 ADBPG 数据库执行一次本脚本完成注册。
--   2. 删除范围为上月月初（含）至本月月初（不含）。
--   3. 删除成功后再运行对应 BB monthly-cdc INSERT 作业。
--********************************************************************--

CREATE OR REPLACE FUNCTION dws.fn_quantum_bb_monthly_delete()
RETURNS void
LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    WHERE report_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month')::date
      ...
      AND (special_fee_type IS NULL
           OR special_fee_type NOT IN ('ACTIVE_CARD_ACCOUNT_FEE', 'CHANNEL_FIXED_FEE'));
...
BEGIN
    ...
    FOR existing_job_id IN
        SELECT jobid FROM cron.job WHERE jobname = 'quantum_bb_monthly_delete_2am'
    LOOP
        PERFORM cron.unschedule(existing_job_id);
    END LOOP;

    PERFORM cron.schedule(
        'quantum_bb_monthly_delete_2am',
        cron_expression,
        'SELECT dws.fn_quantum_bb_monthly_delete()'
    );
END;
$register$;
...
FROM cron.job
WHERE jobname = 'quantum_bb_monthly_delete_2am';
