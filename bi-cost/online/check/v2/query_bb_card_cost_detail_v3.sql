--********************************************************************--
-- Author:         Codex
-- Created Time:   2026-08-17
-- Description:    量子卡 BB V3 渠道成本明细查询，增加渠道返现核对项
-- Usage:
...
bb_check_item AS (
    SELECT 901 AS sort_no, 'BB Channel Cashback Base' AS cost_item, bb_channel_cashback_comm AS cost_amount FROM bb_detail
    UNION ALL SELECT 902, 'BB Cashback Income', cashback_income FROM bb_detail
),
result_detail AS (
    SELECT
        sort_no,
