...
--   v2 说明：渠道成本读取 dws.mv_channel_cost_daily_v2；treasury 无渠道成本时按 0 计算。

-- ==============================================
-- 1. 创建普通物化视图
-- ==============================================
DROP MATERIALIZED VIEW IF EXISTS "dws"."mv_gross_profit_daily_v2";
...
FROM (
    ...
    FULL OUTER JOIN (
        ...
        FROM (
            ...
            FROM "dws"."mv_channel_cost_daily_v2"
...
COMMENT ON MATERIALIZED VIEW "dws"."mv_gross_profit_daily_v2" IS
    'DWS普通物化视图：客户产品线毛利日汇总 v2，收入来源有效收入 MV，成本来源 mv_channel_cost_daily_v2，treasury 无成本时按 0 计算';

-- ==============================================
-- 2. 维护说明
-- ==============================================
-- 本视图为普通物化视图，数据不会自动增量维护。
-- 全量刷新：REFRESH MATERIALIZED VIEW "dws"."mv_gross_profit_daily_v2";
-- 并发刷新：REFRESH MATERIALIZED VIEW CONCURRENTLY "dws"."mv_gross_profit_daily_v2";
