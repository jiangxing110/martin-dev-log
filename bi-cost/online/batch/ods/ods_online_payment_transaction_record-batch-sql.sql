...
CREATE TEMPORARY TABLE flink_source_payment_transaction_record (
    ...
    'table-name' = '(SELECT "id", "create_time", "update_time", "delete_time", "version", "remarks"::text AS "remarks", "source_transaction_id"::text AS "source_transaction_id", "inquiry_id", "business_type"::text AS "business_type", "account_id"::text AS "account_id", "status"::text AS "status", "channel"::text AS "channel", "third_party_payment_id"::text AS "third_party_payment_id", "from_amount", "from_currency"::text AS "from_currency", "to_amount", "to_currency"::text AS "to_currency", "settle_amount", "rate", "payee_id", "parent_id", "same_name_payment", "balance_id"::text AS "balance_id", "settle_currency"::text AS "settle_currency", "fee", "balance_channel"::text AS "balance_channel", "payout_direction_type"::text AS "payout_direction_type", "third_party_reason"::text AS "third_party_reason", "refund_amount", "refund_rate", "transaction_display_id"::text AS "transaction_display_id", "webhook_status"::text AS "webhook_status", "payout_mode"::text AS "payout_mode", "extra"::text AS "extra" FROM public."payment_transaction_record" WHERE (("create_time" >= CAST(''${start_time}'' AS TIMESTAMP(6)) AND "create_time" < CAST(''${end_time}'' AS TIMESTAMP(6))) OR ("update_time" >= CAST(''${start_time}'' AS TIMESTAMP(6)) AND "update_time" < CAST(''${end_time}'' AS TIMESTAMP(6)))) ) AS ods_online_payment_transaction_record_f',
