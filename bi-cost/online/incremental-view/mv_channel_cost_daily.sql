...
SELECT
    ...
    sale_id,
    am_id,
    SUM(acquiring_cost_amount) AS acquiring_cost,
    SUM(business_cost_amount) AS business_cost,
    SUM(quantum_cost_amount) AS quantum_cost,
    SUM(crypto_cost_amount) AS crypto_cost,
    SUM(total_cost_amount) AS total_channel_cost,
    1 AS version,
    NULL::text AS remarks,
    MIN(source_create_time) AS create_time,
    MAX(source_update_time) AS update_time,
...
FROM (
    ...
    SELECT
        ...
        (
            CASE
                WHEN UPPER(TRIM(fc.product_line)) = 'ACQUIRING'
                    THEN COALESCE(fc.cost_amount, 0::numeric)
                ELSE 0::numeric
            END
        )::numeric(20, 4) AS acquiring_cost_amount,
        (
            CASE
                WHEN UPPER(TRIM(fc.product_line)) = 'GLOBAL_ACCOUNT'
                    THEN COALESCE(fc.cost_amount, 0::numeric)
                ELSE 0::numeric
            END
        )::numeric(20, 4) AS business_cost_amount,
        (
            CASE
                WHEN UPPER(TRIM(fc.product_line)) = 'QUANTUM_CARD'
                    THEN COALESCE(fc.cost_amount, 0::numeric)
                ELSE 0::numeric
            END
        )::numeric(20, 4) AS quantum_cost_amount,
        (
            CASE
                WHEN UPPER(TRIM(fc.product_line)) = 'CRYPTO_ASSET'
                    THEN COALESCE(fc.cost_amount, 0::numeric)
                ELSE 0::numeric
            END
        )::numeric(20, 4) AS crypto_cost_amount,
...
CREATE INDEX IF NOT EXISTS "idx_mv_channel_cost_daily_id"
