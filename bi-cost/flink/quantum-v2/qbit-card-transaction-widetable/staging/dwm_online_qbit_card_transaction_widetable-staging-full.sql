...
-- Description:    STAGING 量子卡交易大宽表每日全量同步（多 JDBC source）
-- 前置条件：首次运行前，在 STAGING 数据库执行
--   dwm_qbit_card_transaction_widetable_p-staging-widen.sql
--   将目标表非主键字符串字段扩为 text，避免历史长值写入失败。
--********************************************************************--
...
-- 交易 source：全量读取交易事实，不在 JDBC 查询中关联维度。
...
CREATE TEMPORARY TABLE sink_qbit_card_transaction_widetable (
    ...
    'connector' = 'adbpg', 'url' = 'jdbc:postgresql://${secret_values.STAGING_PG_ASSETS_HOST}:${secret_values.STAGING_PG_ASSETS_PORT}/${secret_values.STAGING_PG_ASSETS_DATABASE}?stringtype=unspecified',
    'targetSchema' = 'dwm', 'tableName' = 'dwm_quantum_card_transaction_p', 'userName' = '${secret_values.STAGING_PG_ASSETS_USERNAME}',
    'password' = '${secret_values.STAGING_PG_ASSETS_PASSWORD}', 'writeMode' = 'upsert', 'batchSize' = '2000', 'retryWaitTime' = '5000'
);
