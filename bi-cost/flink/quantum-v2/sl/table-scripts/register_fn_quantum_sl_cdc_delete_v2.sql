...
-- Description:    注册 SL quantum-v2 CDC v2 删除函数
-- 作业元信息：
--   作业类型：ADBPG 函数注册
...
CREATE OR REPLACE FUNCTION dws.fn_delete_sl_card_finance_daily_v2_cdc(p_dry_run BOOLEAN DEFAULT false)
RETURNS BIGINT
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    IF p_dry_run THEN
        WITH changed_months AS (
            SELECT DISTINCT DATE_TRUNC('month', settlement_date)::date AS report_month
            FROM dwm.dwm_sl_card_transaction_detail_p
...
CREATE OR REPLACE FUNCTION dws.fn_delete_sl_channel_fixed_fee_v2_cdc(p_dry_run BOOLEAN DEFAULT false)
RETURNS BIGINT
LANGUAGE plpgsql
AS $function$
DECLARE
...
SELECT
    dws.fn_delete_sl_card_finance_daily_v2_cdc(true) AS sl_card_finance_daily_matched_rows,
    dws.fn_delete_sl_channel_fixed_fee_v2_cdc(true) AS sl_channel_fixed_fee_matched_rows;
