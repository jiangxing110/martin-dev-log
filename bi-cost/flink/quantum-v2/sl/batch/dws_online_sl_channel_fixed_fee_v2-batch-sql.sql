...
-- Updated Time:   2026-07-30 16:09:22
-- Description:    SL 渠道固定成本批量回刷
--********************************************************************--
...
CREATE TEMPORARY TABLE source_dws_sl_card_finance_daily_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT id, report_date, account_id, account_type, account_category, system_type, sale_id, am_id, rebate_amt, special_fee_type, delete_time FROM dws.dws_sl_card_finance_daily_p WHERE report_date >= CAST(''${start_time}'' AS date) AND report_date < CAST(''${end_time}'' AS date)) AS dws_sl_card_finance_daily_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
...
FROM (
    ...
    WHERE tag = 'CHANNEL_COST' AND provider = 'LS'
      AND statistics_time >= CAST('${start_time}' AS TIMESTAMP(6)) AND statistics_time < CAST('${end_time}' AS TIMESTAMP(6))
) m
