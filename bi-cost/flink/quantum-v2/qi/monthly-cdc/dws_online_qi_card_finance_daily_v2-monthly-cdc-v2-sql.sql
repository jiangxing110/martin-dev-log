...
-- Notes:
--   1. v2 在同一个 Flink SQL 作业中通过 JDBC source 调用 dws.fn_delete_qi_card_finance_daily_v2_monthly_cdc(false) 先清理目标数据。
...
CREATE TEMPORARY TABLE source_delete_qi_card_finance_daily_v2_monthly_cdc_result (
    affected_rows BIGINT
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dws.fn_delete_qi_card_finance_daily_v2_monthly_cdc(false) AS affected_rows) AS delete_result',
...
CROSS JOIN source_delete_qi_card_finance_daily_v2_monthly_cdc_result AS delete_result
