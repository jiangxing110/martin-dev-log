...
SELECT
    ...
    CAST(SUM(CASE
        WHEN status IN ('Closed', 'Pending') AND is_hk_region = FALSE AND business_type IN ('Consumption', 'Reversal', 'Credit')
            THEN billing_amount * CASE WHEN business_type = 'Consumption' THEN 1 ELSE -1 END * CAST(0.02 AS DECIMAL(20, 4))
        ELSE CAST(0 AS DECIMAL(20, 4))
    END) AS DECIMAL(20, 4)) AS rebate_interchange_base_amt,
    CAST(SUM(CASE
        WHEN status IN ('Closed', 'Pending') AND is_hk_region = FALSE AND business_type IN ('Consumption', 'Reversal', 'Credit')
            THEN billing_amount * CASE WHEN business_type = 'Consumption' THEN 1 ELSE -1 END * CAST(0.0118 AS DECIMAL(20, 4))
        ELSE CAST(0 AS DECIMAL(20, 4))
