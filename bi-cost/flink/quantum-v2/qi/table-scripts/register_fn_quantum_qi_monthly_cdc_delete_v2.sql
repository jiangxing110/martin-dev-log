...
-- Description:    注册 QI quantum-v2 monthly-cdc v2 删除函数
-- 作业元信息：
--   作业类型：ADBPG 函数注册
...
CREATE OR REPLACE FUNCTION dws.fn_delete_qi_card_finance_daily_v2_monthly_cdc(p_dry_run BOOLEAN DEFAULT false)
RETURNS BIGINT
LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION dws.fn_delete_qi_channel_fixed_fee_v2_monthly_cdc(p_dry_run BOOLEAN DEFAULT false)
RETURNS BIGINT
LANGUAGE plpgsql
AS $function$
DECLARE
...
SELECT
    dws.fn_delete_qi_card_finance_daily_v2_monthly_cdc(true) AS qi_card_finance_daily_matched_rows,
    dws.fn_delete_qi_channel_fixed_fee_v2_monthly_cdc(true) AS qi_channel_fixed_fee_matched_rows;
