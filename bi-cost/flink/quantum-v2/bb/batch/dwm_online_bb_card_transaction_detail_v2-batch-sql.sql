...
CREATE TEMPORARY TABLE source_bb_quantum_card_transaction_extend_tx (
    ...
    'table-name' = '(SELECT t.id, t.source_id, t.card_transaction_id::text AS card_transaction_id, t.account_id::text AS account_id, t.country, t.type AS "type", t.transaction_time, t.original_completion_time, CAST(t.business_code_list AS text) AS business_code_list, t.remarks, t.card_id::text AS card_id, t.detail, t.create_time, t.update_time, c."type" AS card_org FROM public.quantum_card_transaction_extend_p t INNER JOIN public."qbitCard" c ON t.card_id = c."id" WHERE t.channel_provision = ''BLUEBANC'' AND t.delete_time IS NULL AND t.type IN (''Consumption'', ''Credit'') AND c."type" IN (''Master'', ''VISA'') AND (t.detail IS NULL OR t.detail NOT LIKE ''AUTO CLASS CAR RENTAL%'') AND t.transaction_time >= CAST(''${start_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR AND t.transaction_time < CAST(''${end_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR) AS quantum_card_transaction_extend_tx_f',
...
CREATE TEMPORARY TABLE source_qbit_card_settlement_tx (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT s.id, s.transaction_id, s.qbit_card_transaction_id, s.transaction_type, s.billing_amount, s.raw_data, s.create_time FROM (SELECT "id"::text AS id, "transactionId"::text AS transaction_id, "qbitCardTransactionId"::text AS qbit_card_transaction_id, "transactionType" AS transaction_type, "billingAmount" AS billing_amount, CAST("rawData" AS text) AS raw_data, "createTime" AS create_time, "deleteTime" AS delete_time, "provider" AS provider FROM public."qbitCardSettlement" WHERE "createTime" >= CAST(''${start_time}'' AS TIMESTAMP(6)) - INTERVAL ''1'' MONTH AND "createTime" < CAST(''${end_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' DAY AND CAST("rawData" AS text) NOT LIKE ''%\\u0000%'') s WHERE s.provider = ''BlueBancCard'' AND EXISTS (SELECT 1 FROM public.quantum_card_transaction_extend_p t INNER JOIN public."qbitCard" c ON t.card_id = c."id" WHERE t.channel_provision = ''BLUEBANC'' AND t.delete_time IS NULL AND t.type IN (''Consumption'', ''Credit'') AND c."type" IN (''Master'', ''VISA'') AND (t.detail IS NULL OR t.detail NOT LIKE ''AUTO CLASS CAR RENTAL%'') AND t.transaction_time >= CAST(''${start_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR AND t.transaction_time < CAST(''${end_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR AND t.source_id = s.transaction_id)) AS qbit_card_settlement_tx_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000',
...
CREATE TEMPORARY TABLE source_qbit_card_settlement_oc (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT s.id, s.transaction_id, s.qbit_card_transaction_id, s.transaction_type, s.billing_amount, s.raw_data, s.create_time FROM (SELECT "id"::text AS id, "transactionId"::text AS transaction_id, "qbitCardTransactionId"::text AS qbit_card_transaction_id, "transactionType" AS transaction_type, "billingAmount" AS billing_amount, CAST("rawData" AS text) AS raw_data, "createTime" AS create_time, "deleteTime" AS delete_time, "provider" AS provider FROM public."qbitCardSettlement" WHERE "createTime" >= CAST(''${start_time}'' AS TIMESTAMP(6)) - INTERVAL ''1'' MONTH AND "createTime" < CAST(''${end_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' DAY AND CAST("rawData" AS text) NOT LIKE ''%\\u0000%'') s WHERE s.provider = ''BlueBancCard'' AND EXISTS (SELECT 1 FROM public.quantum_card_transaction_extend_p t INNER JOIN public."qbitCard" c ON t.card_id = c."id" WHERE t.channel_provision = ''BLUEBANC'' AND t.delete_time IS NULL AND t.type IN (''Consumption'', ''Credit'') AND c."type" IN (''Master'', ''VISA'') AND (t.detail IS NULL OR t.detail NOT LIKE ''AUTO CLASS CAR RENTAL%'') AND ((t.transaction_time >= CAST(''${start_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR AND t.transaction_time < CAST(''${end_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR) OR (t.original_completion_time >= CAST(''${start_time}'' AS TIMESTAMP(6)) AND t.original_completion_time < CAST(''${end_time}'' AS TIMESTAMP(6)))) AND t.card_transaction_id::text = s.qbit_card_transaction_id)) AS qbit_card_settlement_oc_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000',
...
CREATE TEMPORARY TABLE source_qbit_card_settlement_refund_direct (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT DISTINCT s.id, s.transaction_id, s.qbit_card_transaction_id, s.transaction_type, s.billing_amount, s.raw_data, s.create_time FROM (SELECT "id"::text AS id, "transactionId"::text AS transaction_id, "qbitCardTransactionId"::text AS qbit_card_transaction_id, "transactionType" AS transaction_type, "billingAmount" AS billing_amount, CAST("rawData" AS text) AS raw_data, "createTime" AS create_time, "deleteTime" AS delete_time, "provider" AS provider FROM public."qbitCardSettlement" WHERE "createTime" >= CAST(''${start_time}'' AS TIMESTAMP(6)) - INTERVAL ''1'' MONTH AND "createTime" < CAST(''${end_time}'' AS TIMESTAMP(6)) + INTERVAL ''8'' DAY AND CAST("rawData" AS text) NOT LIKE ''%\\u0000%'') s INNER JOIN public.quantum_card_transaction_extend_p t ON t.card_transaction_id::text = s.qbit_card_transaction_id INNER JOIN public."qbitCard" c ON c."id" = t.card_id WHERE t.channel_provision = ''BLUEBANC'' AND t.delete_time IS NULL AND t.type = ''Credit'' AND c."type" IN (''Master'', ''VISA'') AND (t.detail IS NULL OR t.detail NOT LIKE ''AUTO CLASS CAR RENTAL%'') AND s.provider = ''BlueBancCard'' AND s.transaction_type = ''refund.clearing'' AND s.raw_data::json->>''responseCode'' = ''APPROVE'' AND CAST(s.raw_data::json->>''postDate'' AS timestamp) >= CAST(''${start_time}'' AS TIMESTAMP(6)) AND CAST(s.raw_data::json->>''postDate'' AS timestamp) < CAST(''${end_time}'' AS TIMESTAMP(6))) AS qbit_card_settlement_refund_direct_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000',
...
WHERE create_time >= CAST('${start_time}' AS TIMESTAMP(6)) - INTERVAL '1' MONTH
  AND create_time < CAST('${end_time}' AS TIMESTAMP(6)) + INTERVAL '8' DAY;
