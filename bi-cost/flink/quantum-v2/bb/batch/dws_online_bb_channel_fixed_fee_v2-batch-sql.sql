...
CREATE TEMPORARY TABLE source_dws_bb_card_finance_daily_v2_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT id, report_date, account_id, account_type, account_category, system_type, sale_id, am_id, total_net_amount, special_fee_type, delete_time FROM dws.dws_bb_card_finance_daily_v2_p WHERE report_date >= CAST(''${start_time}'' AS date) AND report_date < CAST(''${end_time}'' AS date)) AS dws_bb_card_finance_daily_v2_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
