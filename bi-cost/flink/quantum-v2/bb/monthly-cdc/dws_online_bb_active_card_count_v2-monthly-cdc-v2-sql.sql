...
-- 作业元信息：
--   作业类型：批处理
--   运行方式：每月固定调度，按上月完整窗口重算 active_card_count 特殊行
--   运行参数：无，自动取上月完整窗口
-- Notes:
...
CREATE TEMPORARY TABLE source_dwm_bb_card_auth_detail_v2_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT id, card_proxy, account_id, account_type, account_category, system_type, auth_time, update_time, delete_time FROM dwm.dwm_bb_card_auth_detail_v2_p WHERE auth_time >= date_trunc(''month'', CURRENT_DATE - INTERVAL ''1 month'') AND auth_time < date_trunc(''month'', CURRENT_DATE)) AS dwm_bb_card_auth_detail_v2_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
...
CREATE TEMPORARY TABLE source_month_scope (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT CAST(gs.month_start AS DATE) AS report_month FROM generate_series(date_trunc(''month'', date_trunc(''month'', CURRENT_DATE - INTERVAL ''1 month'')), date_trunc(''month'', date_trunc(''month'', CURRENT_DATE)) - INTERVAL ''1 month'', INTERVAL ''1 month'') AS gs(month_start)) AS month_scope_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '100'
...
CREATE TEMPORARY TABLE source_dws_bb_card_finance_daily_v2_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT id, report_date, account_id, account_type, account_category, system_type, sale_id, am_id, special_fee_type, remarks, delete_time FROM dws.dws_bb_card_finance_daily_v2_p WHERE report_date >= date_trunc(''month'', CURRENT_DATE - INTERVAL ''1 month'')::date AND report_date < date_trunc(''month'', CURRENT_DATE)::date) AS dws_bb_card_finance_daily_v2_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '5000'
