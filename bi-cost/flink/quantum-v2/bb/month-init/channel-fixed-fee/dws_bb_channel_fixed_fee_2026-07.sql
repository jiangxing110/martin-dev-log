...
CREATE TEMPORARY TABLE source_dws_bb_card_finance_daily_v2_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT id, report_date, account_id, account_type, account_category, system_type, sale_id, am_id, total_net_amount, special_fee_type, delete_time FROM dws.dws_bb_card_finance_daily_v2_p WHERE report_date >= CAST(''2026-07-01 00:00:00'' AS date) AND report_date < CAST(''2026-08-01 00:00:00'' AS date)) AS dws_bb_card_finance_daily_v2_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
...
FROM (
    ...
    WHERE report_date >= CAST('2026-07-01 00:00:00' AS DATE)
      AND report_date < CAST('2026-08-01 00:00:00' AS DATE)
    UNION
    ...
    WHERE tag = 'CHANNEL_COST' AND provider = 'BB'
      AND statistics_time >= CAST('2026-07-01 00:00:00' AS TIMESTAMP(6)) AND statistics_time < CAST('2026-08-01 00:00:00' AS TIMESTAMP(6))
) m
...
CREATE TEMPORARY TABLE source_delete_bb_channel_fixed_fee_monthly_result (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dws.fn_delete_bb_channel_fixed_fee_v2_cdc(CAST(''2026-07-01'' AS date), CAST(''2026-08-01'' AS date), false) AS affected_rows) AS delete_result',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1'
