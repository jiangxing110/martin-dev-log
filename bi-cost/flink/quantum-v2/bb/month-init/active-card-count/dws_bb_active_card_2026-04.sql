...
CREATE TEMPORARY TABLE source_dwm_bb_card_auth_detail_v2_p (
    ...
    'table-name' = '(SELECT id, card_proxy, account_id, account_type, account_category, system_type, auth_time, update_time, delete_time FROM dwm.dwm_bb_card_auth_detail_v2_p WHERE auth_time >= CAST(''2026-04-01 00:00:00'' AS timestamp) AND auth_time < CAST(''2026-05-01 00:00:00'' AS timestamp)) AS dwm_bb_card_auth_detail_v2_p_f',
...
CREATE TEMPORARY TABLE source_month_scope (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT CAST(gs.month_start AS DATE) AS report_month FROM generate_series(date_trunc(''month'', CAST(''2026-04-01 00:00:00'' AS TIMESTAMP(6))), date_trunc(''month'', CAST(''2026-05-01 00:00:00'' AS TIMESTAMP(6))) - INTERVAL ''1 month'', INTERVAL ''1 month'') AS gs(month_start)) AS month_scope_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '100'
...
CREATE TEMPORARY TABLE source_dws_bb_card_finance_daily_v2_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT id, report_date, account_id, account_type, account_category, system_type, sale_id, am_id, special_fee_type, remarks, delete_time FROM dws.dws_bb_card_finance_daily_v2_p WHERE report_date >= CAST(''2026-04-01 00:00:00'' AS date) AND report_date < CAST(''2026-05-01 00:00:00'' AS date)) AS dws_bb_card_finance_daily_v2_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '5000'
...
CREATE TEMPORARY TABLE source_delete_bb_active_card_count_monthly_result (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dws.fn_delete_bb_active_card_count_v2_cdc(CAST(''2026-04-01'' AS date), CAST(''2026-05-01'' AS date), false) AS affected_rows) AS delete_result',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1'
