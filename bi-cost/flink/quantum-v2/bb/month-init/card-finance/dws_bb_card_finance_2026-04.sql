...
CREATE TEMPORARY TABLE source_dwm_bb_card_transaction_detail_v2_p (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT t.id, t.txn_id, t.settlement_id, t.settlement_match_type, t.source_id, t.card_transaction_id, t.account_id, t.account_type, t.account_category, t.system_type, t.card_id, t.transaction_time, t.original_completion_time, t.business_type, t.business_code_list, t.remarks, t.detail, t.card_org, t.tx_country, t.settle_country, t.is_dom, t.resp_code, t.reason_code, t.transaction_type, t.is_valid_settle, t.is_clearing, t.is_reversal, t.is_refund, t.billing_amount, t.settlement_post_date, t.settlement_txn_date, t.sale_id, t.am_id, t.version, t.create_time, t.update_time, t.delete_time FROM dwm.dwm_bb_card_transaction_detail_v2_p t WHERE t.delete_time IS NULL AND ((t.transaction_time >= CAST(''2026-04-01 00:00:00'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR AND t.transaction_time < CAST(''2026-05-01 00:00:00'' AS TIMESTAMP(6)) + INTERVAL ''8'' HOUR) OR (t.original_completion_time >= CAST(''2026-04-01 00:00:00'' AS TIMESTAMP(6)) AND t.original_completion_time < CAST(''2026-05-01 00:00:00'' AS TIMESTAMP(6))) OR (t.settlement_post_date >= CAST(''2026-04-01 00:00:00'' AS TIMESTAMP(6)) AND t.settlement_post_date < CAST(''2026-05-01 00:00:00'' AS TIMESTAMP(6)))) ) AS dwm_bb_card_transaction_detail_v2_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000'
...
WHERE delete_time IS NULL
  AND transaction_time >= CAST('2026-04-01 00:00:00' AS TIMESTAMP(6))
  AND transaction_time < CAST('2026-05-01 00:00:00' AS TIMESTAMP(6));
...
FROM source_dwm_bb_card_transaction_detail_v2_p
WHERE delete_time IS NULL
  AND original_completion_time >= CAST('2026-04-01 00:00:00' AS TIMESTAMP(6))
  AND original_completion_time < CAST('2026-05-01 00:00:00' AS TIMESTAMP(6));
...
FROM source_dwm_bb_card_transaction_detail_v2_p
WHERE delete_time IS NULL
  AND settlement_post_date >= CAST('2026-04-01 00:00:00' AS TIMESTAMP(6))
  AND settlement_post_date < CAST('2026-05-01 00:00:00' AS TIMESTAMP(6));
...
WHERE delete_time IS NULL
  AND auth_time >= CAST('2026-04-01 00:00:00' AS TIMESTAMP(6))
  AND auth_time < CAST('2026-05-01 00:00:00' AS TIMESTAMP(6));
...
CREATE TEMPORARY TABLE source_delete_bb_card_finance_daily_v2_monthly_result (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}',
    'table-name' = '(SELECT dws.fn_delete_bb_card_finance_daily_v2_cdc(CAST(''2026-04-01'' AS date), CAST(''2026-05-01'' AS date), false) AS affected_rows) AS delete_result',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1'
