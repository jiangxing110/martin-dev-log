...
total_channel_cost_daily AS (
    ...
    FROM (
        ...
        FROM "dws"."dws_total_channel_cost_daily_p"
        WHERE delete_time IS NULL
    ) t
    GROUP BY report_date, account_id
),
finance_channel_cost_daily AS (
    SELECT
        report_date,
        account_id,
        CASE product_line
            WHEN 'QUANTUM_CARD' THEN 'qbit_card'
            WHEN 'GLOBAL_ACCOUNT' THEN 'global_account'
            WHEN 'CRYPTO_ASSET' THEN 'crypto_assets'
            WHEN 'ACQUIRING' THEN 'particle_financing'
            ELSE product_line
        END AS category,
        SUM(
            CASE
                WHEN cost_amount IS NULL OR cost_amount::text = 'NaN' THEN 0
                ELSE cost_amount
            END
        ) AS channel_cost_amount
    FROM "dwm"."dwm_finance_channel_cost_p"
    WHERE delete_time IS NULL
    GROUP BY
        report_date,
        account_id,
...
channel_cost_source AS (
    ...
    FROM finance_channel_cost_daily
