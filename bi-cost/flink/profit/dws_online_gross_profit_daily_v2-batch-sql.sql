...
-- Description:    Gross Profit v2 批量初始化/回刷
-- 作业元信息：
--   作业类型：批处理
--   运行方式：一次性初始化/回刷或调度执行
...
--   2. 成本输入来自 dws.dws_total_channel_cost_daily_v2_p，口径对齐 mv_gross_profit_daily_v2.sql
...
CREATE TEMPORARY TABLE source_total_channel_cost_daily (
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}?stringtype=unspecified',
    'table-name' = '(SELECT report_date, account_id, CAST(CASE WHEN (acquiring_cost IS NULL OR CAST(acquiring_cost AS TEXT) = ''NaN'') THEN 0 ELSE acquiring_cost END AS DOUBLE PRECISION) AS acquiring_cost, CAST(CASE WHEN (business_cost IS NULL OR CAST(business_cost AS TEXT) = ''NaN'') THEN 0 ELSE business_cost END AS DOUBLE PRECISION) AS business_cost, CAST(CASE WHEN (quantum_cost IS NULL OR CAST(quantum_cost AS TEXT) = ''NaN'') THEN 0 ELSE quantum_cost END AS DOUBLE PRECISION) AS quantum_cost, CAST(CASE WHEN (crypto_cost IS NULL OR CAST(crypto_cost AS TEXT) = ''NaN'') THEN 0 ELSE crypto_cost END AS DOUBLE PRECISION) AS crypto_cost, CAST(CASE WHEN (total_channel_cost IS NULL OR CAST(total_channel_cost AS TEXT) = ''NaN'') THEN 0 ELSE total_channel_cost END AS DOUBLE PRECISION) AS total_channel_cost FROM dws.dws_total_channel_cost_daily_v2_p WHERE delete_time IS NULL AND report_date >= CAST(''${start_date}'' AS DATE) AND report_date < CAST(''${end_date}'' AS DATE)) AS dws_total_channel_cost_daily_v2_p_f',
    'username' = '${secret_values.ADB_PG_USERNAME}',
    'password' = '${secret_values.ADB_PG_PASSWORD}',
    'driver' = 'org.postgresql.Driver',
    'scan.fetch-size' = '1000'
...
CREATE TEMPORARY VIEW v_revenue_daily AS
SELECT
    report_date,
    account_id,
...
SELECT report_date, account_id, 'qbit_card' AS category, quantum_cost AS channel_cost_amount
FROM v_total_channel_cost_daily
WHERE quantum_cost <> CAST(0 AS DECIMAL(20, 4))
UNION ALL
SELECT report_date, account_id, 'global_account' AS category, business_cost AS channel_cost_amount
FROM v_total_channel_cost_daily
WHERE business_cost <> CAST(0 AS DECIMAL(20, 4))
UNION ALL
SELECT report_date, account_id, 'crypto_assets' AS category, crypto_cost AS channel_cost_amount
FROM v_total_channel_cost_daily
...
CREATE TEMPORARY VIEW v_channel_cost_by_category AS
SELECT
    report_date,
    account_id,
    category,
    CAST(SUM(channel_cost_amount) AS DECIMAL(20, 4)) AS channel_cost_amount
...
SELECT
    ...
    CAST(COALESCE(c.channel_cost_amount, CAST(0 AS DECIMAL(20, 4))) AS DECIMAL(20, 4)) AS channel_cost_amount
FROM v_revenue_daily r
FULL OUTER JOIN v_channel_cost_by_category c
