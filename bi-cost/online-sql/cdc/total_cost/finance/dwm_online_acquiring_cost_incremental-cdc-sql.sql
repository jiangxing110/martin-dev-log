...
-- Description:    金融渠道成本 DWM CDC初始化 - ACQUIRING
-- 作业元信息：
--   作业类型：流处理 CDC
--   运行方式：全量初始化 + 增量实时同步
--   运行参数：无
...
-- v_param 在原批处理中使用硬编码日期（如 2026-05-01）推导月份参数，
-- CDC 版本需根据实际场景调整日期推导逻辑或改用动态参数。
-- 当前保留 v_day_numbers 作为工具视图备用。
...
SELECT
    ...
    b.cost_amount,
    1 AS version,
    CAST('acquiring_batch' AS STRING) AS remarks,
