...
-- Description:    金融渠道成本 DWM CDC初始化 - QUANTUM_CARD / BPC
-- 作业元信息：
--   作业类型：流处理 CDC
--   运行方式：全量初始化 + 增量实时同步
--   运行参数：无
...
SELECT q.account_id
FROM source_qbit_card q
WHERE q.provider LIKE '%Qbit%'
  AND (q.delete_card_time > CAST('2026-05-01' AS TIMESTAMP(6)) OR q.delete_card_time IS NULL)
...
SELECT
    ...
    b.cost_amount,
    1 AS version,
    CAST('quantum_card_bpc_batch' AS STRING) AS remarks,
