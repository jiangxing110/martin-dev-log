...
-- Description:    金融渠道成本 DWM CDC初始化/回刷
-- 作业元信息：
--   作业类型：流处理 CDC
--   运行方式：全量初始化 + 增量实时同步
--   运行参数：无
...
-- 注意：
--   本作业原为批处理模式，依赖 source_month/next_month 参数生成月粒度分摊数据。
-- CDC 版本已移除 v_param 硬编码日期，后续需要根据实际场景重新设计增量分摊逻辑。
-- 当前 SQL 结构保留原分摊算法骨架，v_param 相关引用已被注释或移除，执行前需手动调整。
