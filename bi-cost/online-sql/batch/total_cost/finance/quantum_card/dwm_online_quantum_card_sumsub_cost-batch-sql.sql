...
-- Description:    金融渠道成本 DWM 批量初始化 - QUANTUM_CARD / Sumsub
-- 作业元信息：
--   作业类型：批处理
--   运行方式：一次性初始化/回刷或调度执行
...
WHERE r.request_channel = 'sumsub'
  ...
  AND r.create_time >= CAST('2026-05-01' AS TIMESTAMP(6))
  AND r.create_time < CAST('2026-06-01' AS TIMESTAMP(6))
...
SELECT
    ...
    b.cost_amount,
    1 AS version,
    CAST('quantum_card_sumsub_batch' AS STRING) AS remarks,
