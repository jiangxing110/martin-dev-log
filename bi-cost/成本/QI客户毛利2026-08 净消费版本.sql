-- QI客户毛利2026-08（完整版，增加KYC应收金额已去除，增加DCSF成本，手动实收）
-- VISA另一份账单、活跃卡费、还没出
-- 注释掉：低消应收、月结手续费应收、低消实收、月结手续费实收
SET myapp.start_time = '2026-08-01 00:00:00';
SET myapp.end_time = '2026-09-01 00:00:00';
SET myapp.param_a = '1.0087';
SET myapp.param_b = '0.9220';
SET myapp.param_c = '1.1044';
SET myapp.param_d = '1.0743'; --VRM
SET myapp.param_e = '1.1347'; --CROSS
SET myapp.param_f = '0.9163'; --DCSF
...
transaction_base AS MATERIALIZED (
    ...
    LEFT JOIN quantum_card_transaction_extend_p D ON B."transactionId" = D."transaction_id"
    CROSS JOIN params p
...
receivable_allocation AS (
    WITH bill_ids AS (
        ...
        WHERE delete_time IS NULL AND is_latest='t' AND bill_month='2026-08'--每月调整
    ),
    summary AS (
...
-- ================= 从 provider_collection_received 获取新实收（仅其他手续费） =================
new_receipts AS (
    SELECT 
        AC."id"::text AS "account_id",
        A."displayid" AS "母客户id",
        SUM(A."monthly_fee_received" + A."fx_fee_received") AS "其他手续费实收金额_QI",
        -- 低消和月结实收置零（已注释掉，不再使用）
        0 AS "低消实收金额_QI",
        0 AS "月结手续费实收金额_QI",
        -- 总实收仅包含其他手续费实收（不再包含所有渠道）
        SUM(A."monthly_fee_received" + A."fx_fee_received") AS "总实收金额_QI"
    FROM provider_collection_received A
    LEFT JOIN "account" AC ON AC."displayId" = A."displayid"
    WHERE A."provider" = 'IQ'
      AND A."month" = '2026-08'   -- 每月调整，与bill_month一致
    GROUP BY AC."id", A."displayid"
),
...
combined_costs AS (
    SELECT 
        ...
        CASE 
            WHEN cc."母客户id" = '666245' THEN 0 --每月调整创鑫基本后面都没有量了
...
ProviderTopUpFee AS (
    ...
    CROSS JOIN params p
    WHERE ptf."Collection Month" = '2026-08'   -- 每月调整
...
final_summary AS (
    SELECT 
        ...
        GREATEST((17330+10667+0) * COALESCE(cons."客户QI渠道净消费占比", 0) / 100, 0) as "成本十",--每月调整固定成本+BPC成本+另一份账单的成本
        GREATEST(9500 * COALESCE(cons."客户QI渠道净消费占比", 0) / 100, 0) as "银行手续费成本",--每月调整
				(CASE 
				  WHEN amc."母客户id" = '732600' THEN 0
					WHEN amc."母客户id" = '397244' THEN 0
					WHEN amc."母客户id" = '575652' THEN 0
					WHEN amc."母客户id" = '909164' THEN 0
					WHEN amc."母客户id" = '643403' THEN 0
					WHEN amc."母客户id" = '816475' THEN 0
					WHEN amc."母客户id" = '311205' THEN 0
					WHEN amc."母客户id" = '744844' THEN 0
					WHEN amc."母客户id" = '826129' THEN 0
					WHEN amc."母客户id" = '287317' THEN 0
					WHEN amc."母客户id" = '686357' THEN 0
					WHEN amc."母客户id" = '535533' THEN 0
					WHEN amc."母客户id" = '477228' THEN 0
					WHEN amc."母客户id" = '776244' THEN 0
					WHEN amc."母客户id" = '173874' THEN 0
							ELSE 0 END) * (COALESCE(cons."QI渠道净消费占比", 0) / 100) AS "直客手动退款", --每月调整
				
        (CASE 
            WHEN amc."母客户id" = '440126' THEN 0
						WHEN amc."母客户id" = '689471' THEN 0
						WHEN amc."母客户id" = '490216' THEN 0
						WHEN amc."母客户id" = '388995' THEN 0
						WHEN amc."母客户id" = '656337' THEN 0
						WHEN amc."母客户id" = '683509' THEN 0
        END )* (COALESCE(cons."QI渠道净消费占比", 0) / 100) AS "应收_财务线下手动调整",--每月调整
				
        CASE 
            WHEN amc."母客户id" = '812151' THEN 0
						WHEN amc."母客户id" = '761059' THEN -90244.95 --实体卡退款9000张
						
            ELSE 0
        END AS "线下实体卡收入",--每月调整
				
				CASE
						WHEN amc."母客户id" = '018142' THEN 181.05
						WHEN amc."母客户id" = '079942' THEN 178.05
						WHEN amc."母客户id" = '148807' THEN 175.05
						WHEN amc."母客户id" = '174529' THEN 172.05
						WHEN amc."母客户id" = '236621' THEN 352.05
						WHEN amc."母客户id" = '322383' THEN 178.05
						WHEN amc."母客户id" = '369660' THEN 178.05
						WHEN amc."母客户id" = '426164' THEN 325.05
						WHEN amc."母客户id" = '445691' THEN 271.05
						WHEN amc."母客户id" = '477226' THEN 175.05
						WHEN amc."母客户id" = '477228' THEN 193.05
						WHEN amc."母客户id" = '645329' THEN 181.05
						WHEN amc."母客户id" = '665218' THEN 175.05
						WHEN amc."母客户id" = '679282' THEN 265.05
						WHEN amc."母客户id" = '761059' THEN 36787.05-27000 --实体卡退款9000张
						WHEN amc."母客户id" = '812151' THEN 2272.05
						WHEN amc."母客户id" = '854497' THEN 184.05
						WHEN amc."母客户id" = '870979' THEN 202.05
						WHEN amc."母客户id" = '907519' THEN 313.05
						WHEN amc."母客户id" = '990552' THEN 175.05
						ELSE 0
				END AS "新增实体卡成本",--每月调整
				
        (CASE 
            WHEN amc."母客户id" = '761059' THEN 0 
						WHEN amc."母客户id" = '350441' THEN 0
						WHEN amc."母客户id" = '440126' THEN 0
						WHEN amc."母客户id" = '456274' THEN 0
						WHEN amc."母客户id" = '457615' THEN 0
						WHEN amc."母客户id" = '561004' THEN 0
						WHEN amc."母客户id" = '512599' THEN 0
            ELSE 0
        END) * (COALESCE(cons."QI渠道净消费占比", 0) / 100) AS "财务手动调整返现成本",--每月调整
				
				CASE
						WHEN amc."母客户id" = '018142' THEN 330.35
						WHEN amc."母客户id" = '128139' THEN 100.57
						WHEN amc."母客户id" = '148807' THEN 69
						WHEN amc."母客户id" = '174529' THEN 5.6
						WHEN amc."母客户id" = '322383' THEN 1347.38
						WHEN amc."母客户id" = '426164' THEN 19.88
						WHEN amc."母客户id" = '434023' THEN 84
						WHEN amc."母客户id" = '436731' THEN 10.38
						WHEN amc."母客户id" = '523135' THEN 706.99
						WHEN amc."母客户id" = '562513' THEN 0.45
						WHEN amc."母客户id" = '599703' THEN 71.42
						WHEN amc."母客户id" = '637549' THEN 309.82
						WHEN amc."母客户id" = '838108' THEN 33524.25
						WHEN amc."母客户id" = '990552' THEN 1049.37
						ELSE 0
				END AS "客户返现代收成本", --每月调整
				
        CASE 
            WHEN amc."母客户id" = '990552' THEN 0
            WHEN amc."母客户id" = '183821' THEN 0
            ELSE 0
        END AS "手动计算应收",--每月调整，后续已经优化，目前等我算完财务提交流程已经可以拆分到渠道，后续就不需要我手动再加了
        ...
        -- 实收字段从 new_receipts 获取（仅其他手续费实收）
    ...
    LEFT JOIN new_receipts nar ON amc."母客户id" = nar."母客户id"
...
SELECT 
    ...
    (COALESCE(f."成本一", 0) + COALESCE(f."成本二", 0) + COALESCE(f."成本三", 0) + 
     ...
     COALESCE(f."线下实体卡收入", 0) + 
		 ...
		 COALESCE(f."其他手续费实收金额_QI", 0)+COALESCE(f."直客手动退款", 0)+COALESCE(f."应收_财务线下手动调整", 0)
		 ) AS "Cashflow_Rev",
     ...
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."邮寄费收入", 0) +
     COALESCE(f."线下实体卡收入", 0)+COALESCE(f."直客手动退款", 0)+COALESCE(f."应收_财务线下手动调整", 0)
    ...
    COALESCE(f."其他手续费实收金额_QI", 0) AS "Cashflow_Rev_之前月账单_collected",
     ...
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."邮寄费收入", 0) + COALESCE(f."直客手动退款", 0) +
     COALESCE(f."线下实体卡收入", 0) + COALESCE(f."其他手续费实收金额_QI", 0) +COALESCE(f."直客手动退款", 0)+COALESCE(f."应收_财务线下手动调整", 0)-
		 (COALESCE(f."成本一", 0) + COALESCE(f."成本二", 0) + COALESCE(f."成本三", 0) + 
     COALESCE(f."成本四", 0) + COALESCE(f."成本五", 0) + COALESCE(f."成本六", 0) + 
     COALESCE(f."成本七", 0) + COALESCE(f."成本八", 0) + COALESCE(f."成本九", 0) + 
     COALESCE(f."成本十", 0) + COALESCE(f."银行手续费成本", 0) + 
     ...
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0) + COALESCE(f."其他手续费实收金额_QI", 0)+COALESCE(f."直客手动退款", 0)+COALESCE(f."应收_财务线下手动调整", 0)
		 ) AS "Cashflow_Rev_sale",

		(COALESCE(f."充值费收入_QI", 0) + COALESCE(f."虚拟开卡费收入", 0) + COALESCE(f."FX_Cross费用收入", 0) + 
     COALESCE(f."Settlement_Auth_Fee收入", 0) + COALESCE(f."其他费用收入", 0) + 
     COALESCE(f."新增收入", 0)+COALESCE(f."直客手动退款", 0)+COALESCE(f."应收_财务线下手动调整", 0)
		 ) AS "Cashflow_Rev_realtime_sale",
     ...
     COALESCE(f."成本四", 0) + COALESCE(f."成本五", 0) + COALESCE(f."成本六", 0) + 
     COALESCE(f."成本七", 0) + COALESCE(f."成本八", 0) + COALESCE(f."成本九", 0) + 
     COALESCE(f."成本十", 0) + COALESCE(f."银行手续费成本", 0) + COALESCE(f."财务手动调整返现成本", 0)+
...
WHERE f."母客户id" IS NOT NULL
    AND (
        ...
        OR f."新增收入" != 0 OR f."总实收金额_QI" != 0 OR f."邮寄费收入" != 0 
