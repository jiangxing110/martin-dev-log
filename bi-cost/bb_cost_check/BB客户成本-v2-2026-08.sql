...
-- 已自动按月份推导：日期区间、bb_card_auth_detail_2026-08 分表名。
-- =============================================================================
-- =============================================================================
...
SELECT 
    '2026-08-01'::timestamp AS month_start,          -- 报表月份第一天 00:00:00
    '2026-09-01'::timestamp AS month_end,            -- 报表月份下个月第一天 00:00:00
    '2026-08-01 08:00:00'::timestamp AS txn_start,   -- transaction_time 开始（带时区偏移）
    '2026-09-01 08:00:00'::timestamp AS txn_end,     -- transaction_time 结束（带时区偏移）
    '2026-07-01 00:00:00'::timestamp AS settle_start, -- settle表开始时间前延一个月
    '2026-09-10 00:00:00'::timestamp AS settle_end,   -- settle表结束时间后延一个月
    '2026-08-01'::date AS post_start,                 -- post_date-计算退款 开始时间
    '2026-09-01'::date AS post_end,                   -- post_date-计算退款 结束时间
    '2026-08'::text AS month_label,                   -- 输出月份标签
...
SELECT 
    ...
    C."type" AS card_type,
    A."delete_time"
FROM "quantum_card_transaction_extend_p" A
...
q6 AS (
    ...
    FROM "bb_card_auth_detail_2026-08" A --每月调整
