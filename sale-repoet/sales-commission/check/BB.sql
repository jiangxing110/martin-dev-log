WITH params AS (
    SELECT
        ...
        'd631b89a-82af-4cd6-925e-c95889515a34'::varchar AS root_account_id
),
account_scope AS (
...
bb_base AS (
    SELECT
        ...
        ABS(
            COALESCE(
                SUM(COALESCE(bb.bb_rebate_base_amt, 0) * 0.021195),
                0
            )
        ) AS channel_rebate
...
result_detail AS (
    ...
    -- 渠道返现单独展示，不计入 TOTAL 成本
    UNION ALL
    SELECT
        28,
        'Channel Rebate',
        CAST(channel_rebate AS numeric(20, 4))
    FROM bb_base
