...
Assets 侧拆分为两个 XXL-JOB：

- `partner_gross_margin_commission_job`：保留为毛利返佣单独计算/修数任务。
- `partner_monthly_commission_calculation_job`：新增月度三种返佣统一计算任务，负责组织毛利返佣和两种加点交易处理。

统一计算任务：

执行顺序固定为“先查合伙人，再查客户返佣”：

1. 从 `account` 查询全部 `InterlacePartnerReferral`、`InterlacePartnerSalesAgent`、`InterlacePartnerWhiteLabel` 合伙人。
2. 按每个 `partnerAccountId` 查询其有效邀请码关联的客户快照和返佣交易。
3. 未传 `partnerAccountId` 时处理全部合伙人；传入时只处理指定合伙人。
4. 三种模式使用同一个 `sourceBatchNo` 进入后续推送。
...
当前统一计算任务已经接入 Assets 内部计算服务，但跨项目推送仍需等待公共 API 的批次推送契约和 Partner 接收 Service 完成后接入；在此之前不能把它标记为“已推送”。
...
当前 `pageNo` 固定为 1、`hasNext` 为 false，每个合伙人独立推送完整结果，避免现有 Partner 明细同步逻辑在多页场景下覆盖上一页。后续单个合伙人数据量超过单页上限时，再将 Partner 接收仓储改为追加式分页幂等。
...
- [x] 抽取 Assets 侧三种返佣的统一计算服务。
- [x] 新增 `PartnerMonthlyCommissionCalculationJob`，任务标识为 `partner_monthly_commission_calculation_job`；不新增批次表，批次号由任务参数或本次运行生成。
- [x] 保留 `PartnerGrossMarginCommissionJob` 作为毛利返佣单独计算/修数任务。
- [x] 支持统一计算任务传入指定月份、指定快照日、指定合伙人和 `force` 参数；指定合伙人时毛利和加点交易均按合伙人过滤。
- [x] 计算完成后按合伙人主动推送 Partner；当前每个合伙人一次推送完整结果。
- [x] Partner 按 `sourceBatchNo + partnerAccountId + settlementMonth` 做当前单页接收幂等；Assets 任务失败后复用同一批次重试，已成功合伙人自动跳过。
...
- [ ] 新增批次创建请求和响应 DTO。
- [x] 新增批次推送请求和响应 DTO。
- [x] 新增 Partner 批次推送 Feign 接口。
- [ ] 清理旧的月度结算主动查询契约：确认 `PartnerCommissionClient#queryPartnerCommissionForSettlement` 仅被旧结算拉取流程使用后，再从公共 API、Assets Controller 和 Partner 调用方一起删除。
- [ ] 保留首页仍使用的预估、KYC 和其他查询契约，不能随旧结算接口一起删除。
- [x] 补充三种返佣结果、合伙人信息、推荐客户数和批次号字段。
- [ ] 发布公共 API 版本，并同步更新 Assets、Partner 依赖版本。

### 10.3 Partner 项目

- [x] 新增批次推送 Controller，Controller 只负责参数接收和结果返回。
- [x] 在 Service 中完成批次校验、当前单页幂等校验和主表/详情表落库。
...
- [x] 删除旧的主动拉取 Job；后续由 Assets 计算完成后主动推送 Partner。
- [ ] 删除旧的 Assets 月度结算主动查询 Feign 接口和调用代码；首页预估、KYC 等仍使用的查询接口继续保留。
- [ ] 删除 Partner 侧旧的毛利返佣汇总计算逻辑。
- [ ] 补充推送接收、重复页面、断点重试和事务回滚测试。

### 10.3.1 非毛利账单明细映射

`partner_monthly_commission_detail` 已按照渠道月度佣金账单 Excel 的明细结构设计，底价加价和弹性加价不需要新增表。Excel 中的两个非毛利返佣明细属于 Assets 统一返佣结果的一部分，与毛利返佣在同一批次中推送，Partner 不再单独查询或补数据。Assets 推送数据时，Partner 必须写入以下现有字段：
...
当前同步代码已写入客户、产品线、费用类型、分佣模式、佣金金额和批次号，但部分 Excel 展示字段尚未从 Assets DTO 传递并落库。后续只补充统一推送 DTO、查询结果模型、Partner 领域明细模型和现有 Entity 映射，不调整 `partner_monthly_commission_detail` 表结构。

### 10.4 首页数据范围（P0，最高优先级）

- [x] P0：增加当前登录用户 `userId` 和 `accountId` 的范围解析。
- [x] P0：`userType = Agent` 时只查询当前 `userId` 关联的邀请码。
- [x] P0：非 `Agent` 时查询当前 `accountId` 下全部有效邀请码。
- [x] P0：将统一邀请码范围应用到余额、KYC、白标客户和量子卡趋势接口。
...
- [x] 确认同一批次重复推送不会重复落库；当前每个合伙人仅推送第 1 页，幂等键包含合伙人账户和结算月。
- [ ] 确认历史月份可以通过新批次重新推送修复。
