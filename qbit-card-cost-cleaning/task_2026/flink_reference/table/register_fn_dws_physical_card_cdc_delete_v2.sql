...
LEFT JOIN "qbitCard" AS qc ON tr."cardId" = qc."id"
        ...
        LOOP
            IF p_dry_run THEN
                EXECUTE format($fmt$SELECT COUNT(*) FROM public.dws_physical_card_%s WHERE (account_id, provider, bin, status, create_date) IN (SELECT DISTINCT tr."accountId", qc."provider", qc."firstSix", tr."status", DATE(tr."createTime") FROM "qbitCardWalletTransaction" AS tr
LEFT JOIN "qbitCard" AS qc ON tr."cardId" = qc."id" WHERE (tr."createTime" >= CURRENT_DATE - INTERVAL '1 day' AND tr."createTime" < CURRENT_DATE))$fmt$, v_year) INTO v_n;
            ELSE
                EXECUTE format($fmt$DELETE FROM public.dws_physical_card_%s WHERE (account_id, provider, bin, status, create_date) IN (SELECT DISTINCT tr."accountId", qc."provider", qc."firstSix", tr."status", DATE(tr."createTime") FROM "qbitCardWalletTransaction" AS tr
