...
LEFT JOIN "qbitCard" AS qc ON tr."cardId" = qc."id"
        ...
        LOOP
            IF p_dry_run THEN
                EXECUTE format($fmt$SELECT COUNT(*) FROM public.dws_qbit_card_transaction_%s WHERE (account_id, provider, bin, business_type, create_date, status) IN (SELECT DISTINCT tr."accountId", tr."provider", qc."firstSix", tr."businessType", DATE_TRUNC('day', tr."createTime")::TIMESTAMP, tr."status" FROM "qbit_card_transaction" AS tr
LEFT JOIN "qbitCard" AS qc ON tr."cardId" = qc."id" WHERE (tr."createTime" >= CURRENT_DATE - INTERVAL '1 day' AND tr."createTime" < CURRENT_DATE))$fmt$, v_year) INTO v_n;
            ELSE
                EXECUTE format($fmt$DELETE FROM public.dws_qbit_card_transaction_%s WHERE (account_id, provider, bin, business_type, create_date, status) IN (SELECT DISTINCT tr."accountId", tr."provider", qc."firstSix", tr."businessType", DATE_TRUNC('day', tr."createTime")::TIMESTAMP, tr."status" FROM "qbit_card_transaction" AS tr
