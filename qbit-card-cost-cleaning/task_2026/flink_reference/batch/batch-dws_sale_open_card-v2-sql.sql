...
CREATE TEMPORARY TABLE source_delete_dws_sale_open_card_result (
    ...
    'table-name' = '(SELECT public.fn_delete_dws_sale_open_card_cdc(false, DATE ''2026-01-01'', DATE ''2026-08-17'') AS affected_rows) AS delete_result',
...
AND tr."createTime" >= CURRENT_DATE - INTERVAL '1 day' AND tr."createTime" < CURRENT_DATE
    ...
    SELECT tr."accountId", qc.provider, qc."firstSix", tr."status", ids."sale_or_am_id", COALESCE(sum("senderFee"),0) fee, count(*) count, TO_CHAR(tr."createTime", 'YYYY-MM-DD')::DATE AS "create_date", 1 AS "version", -- 初始版本号
...
CREATE TEMPORARY TABLE sink_dws_sale_open_card_2027 (
    id BIGINT, account_id STRING, provider STRING, bin STRING, status STRING, sale_or_am_id STRING, fee DECIMAL(20,4), count BIGINT, create_date DATE, version BIGINT, create_time TIMESTAMP(6), update_time TIMESTAMP(6),
    PRIMARY KEY (id) NOT ENFORCED
) WITH ('connector'='adbpg','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}','tableName'='public.dws_sale_open_card_2027','userName'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','writeMode'='upsert','batchSize'='2000');
...
INSERT INTO sink_dws_sale_open_card_2027
SELECT id, account_id, provider, bin, status, sale_or_am_id, fee, count, create_date, version, create_time, update_time
FROM v_dws_sale_open_card_base
CROSS JOIN source_delete_dws_sale_open_card_result AS del
WHERE del.affected_rows >= 0
