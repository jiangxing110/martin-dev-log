...
-- Description:    dws_sale_open_card 批处理 作业（quantum-v2 范式：确定性哈希主键 + 先清后写）
-- 作业元信息：
--   作业类型：批处理
...
CREATE TEMPORARY TABLE source_delete_dws_sale_open_card_result (
    ...
    'table-name' = '(SELECT public.fn_delete_dws_sale_open_card_cdc(false, CAST(''${start_date}'' AS DATE), CAST(''${end_date}'' AS DATE)) AS affected_rows) AS delete_result',
