...
CREATE TEMPORARY TABLE sink_dws_sale_transfer_2027 (
    id BIGINT, account_id STRING, sale_or_am_id STRING, business_type_detail STRING, business_type_code STRING, settlement_currency STRING, status STRING, usd_amount DECIMAL(20,4), transaction_count BIGINT, fee DECIMAL(20,4), currency STRING, create_date DATE, version BIGINT, create_time TIMESTAMP(6), update_time TIMESTAMP(6),
    PRIMARY KEY (id) NOT ENFORCED
) WITH ('connector'='adbpg','url'='jdbc:postgresql://${secret_values.ADB_PG_VPC_HOSTNAME}:${secret_values.ADB_PG_VPC_PORT}/${secret_values.ADB_PG_DATABASE}','tableName'='public.dws_sale_transfer_2027','userName'='${secret_values.ADB_PG_USERNAME}','password'='${secret_values.ADB_PG_PASSWORD}','writeMode'='upsert','batchSize'='2000');
...
INSERT INTO sink_dws_sale_transfer_2027
SELECT id, account_id, sale_or_am_id, business_type_detail, business_type_code, settlement_currency, status, usd_amount, transaction_count, fee, currency, create_date, version, create_time, update_time
FROM v_dws_sale_transfer_base
CROSS JOIN source_delete_dws_sale_transfer_result AS del
WHERE del.affected_rows >= 0
