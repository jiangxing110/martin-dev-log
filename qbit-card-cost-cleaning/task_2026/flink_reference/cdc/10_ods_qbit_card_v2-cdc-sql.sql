...
-- Description:    ods_qbit_card 流处理(CDC) 作业（quantum-v2 范式：确定性哈希主键 + 先清后写）
-- 作业元信息：
--   作业类型：流处理(CDC)
...
CREATE TEMPORARY TABLE source_delete_ods_qbit_card_result (
    ...
    'table-name' = '(SELECT public.fn_delete_ods_qbit_card_cdc(false) AS affected_rows) AS delete_result',
...
--    （source_ods_qbit_card 已输出聚合列，此处只补 id；列名与 DWS 表一致）
-- ==============================================
