...
public class DailySettleJob {
    ...
    private static final int DEFAULT_ACCOUNT_CONCURRENCY = 4;
    ...
    public void processDailySettle(String settleDate, String accountId) {
        ...
        /**
         * 自动生成日账单的“幂等集合”：
         * 同一天内同一个 accountId 生成成功/判定无 handler 后，都会写入该集合，
         * 后续扫描到会直接跳过，避免重复生成与重复消耗数据库连接。
         */
        String processedSetKey = "qbit:assets:daily-settle:processed:" + settleDate;
        String lastId = null;

        // ====== 统计口径（只用于日志汇总，不影响业务逻辑） ======
        // 已回收并统计的账户数量（GENERATED/NO_HANDLER/FAILED 的总和）
        int totalScanned = 0;
        // 已成功生成并上传账单的账户数量
        int totalGenerated = 0;
        // 已在 processedSetKey 中的账户数量（本次直接跳过）
        int totalSkipped = 0;
        // 账户存在但无匹配 handler（且非 distributor）的数量
        int totalNoHandler = 0;
        // 处理失败的账户数量（账户不存在/生成异常等）
        int totalFailed = 0;

        // 账户维度“滑动窗口”并发：任何时刻最多处理 accountConcurrency 个账户，
        // 完成一个立即补一个，避免把整批任务塞进队列导致内存/连接堆积。
        int accountConcurrency = resolveAccountConcurrency();
        ExecutorService accountExecutor = Executors.newFixedThreadPool(accountConcurrency, r -> {
            Thread t = new Thread(r);
            t.setName("daily-statement-account-" + t.getId());
            t.setDaemon(true);
            return t;
        });
        CompletionService<AccountProcessResult> completionService = new ExecutorCompletionService<>(accountExecutor);
        int inFlight = 0;
        try {
            while (true) {
                ...
                for (String accId : accountIds) {
                    ...
                    // 滑动窗口：达到上限就等待一个完成，再继续提交
                    if (inFlight >= accountConcurrency) {
                        AccountProcessResult result = takeCompleted(completionService, settleDate);
                        if (result != null) {
                            totalScanned++;
                            switch (result.status) {
                                case GENERATED -> totalGenerated++;
                                case NO_HANDLER -> totalNoHandler++;
                    ...
                    // 提交一个账户任务；任务内部自行写入 processedSetKey（成功/无 handler）
                    completionService.submit(() -> processSingleAccountForAuto(accId, settleDate, processedSetKey));
                    inFlight++;
                }
    ...
    private enum AccountProcessStatus {
        GENERATED,
        NO_HANDLER,
        FAILED
    }

    private record AccountProcessResult(String accountId, AccountProcessStatus status) {
    }

    /**
     * 自动日账单单账户处理（并发任务）。
     */
    private AccountProcessResult processSingleAccountForAuto(String accountId, String settleDate, String processedSetKey) {
        try {
            Account account = accountMapper.selectById(accountId);
            if (account == null) {
                log.warn("账户不存在: accountId={}", accountId);
                return new AccountProcessResult(accountId, AccountProcessStatus.FAILED);
            }
            ...
            if (handlers.isEmpty() && !isDistributor) {
                // 无 handler 的账户记入幂等集合：后续扫描不再反复判定
                redisService.sAdd(processedSetKey, accountId);
                return new AccountProcessResult(accountId, AccountProcessStatus.NO_HANDLER);
            }

            generateDailyStatement(handlers, account, settleDate);
            // 成功生成后写入幂等集合，避免同一天重复生成
            redisService.sAdd(processedSetKey, accountId);
            return new AccountProcessResult(accountId, AccountProcessStatus.GENERATED);
        } catch (Exception e) {
            log.error("处理日账单失败: accountId={}, date={}", accountId, settleDate, e);
            return new AccountProcessResult(accountId, AccountProcessStatus.FAILED);
        }
    }

    private AccountProcessResult takeCompleted(CompletionService<AccountProcessResult> completionService, String settleDate) {
        try {
            Future<AccountProcessResult> future = completionService.take();
            return future.get();
        } catch (Exception e) {
            log.error("回收账户任务失败: date={}", settleDate, e);
            return null;
        }
    }
     ...
     * 获取自动日账单账户级并发数。
     */
    private int resolveAccountConcurrency() {
        try {
            String raw = System.getProperty("daily.statement.account.concurrency");
            if (StringUtils.isBlank(raw)) {
                raw = System.getenv("DAILY_STATEMENT_ACCOUNT_CONCURRENCY");
            }
            if (StringUtils.isBlank(raw)) {
            ...
            log.warn("读取账户级并发配置失败，使用默认值: {}", DEFAULT_ACCOUNT_CONCURRENCY, e);
            return DEFAULT_ACCOUNT_CONCURRENCY;
        }
    }
    ...
    private void uploadZipAndSaveRecord(ByteArrayOutputStream zipBaos, String accountId, String date, int fileCount) {
       ...
       /* DailyStatementFile existing = dailyStatementFileMapper.selectByAccountIdAndDate(accountId, java.sql.Date.valueOf(date));
        ...
        log.warn("日账单 ZIP 已生成: accountId={}, date={}, fileUrl={}, fileCount={}", accountId, date, fileUrl, fileCount);
    }
